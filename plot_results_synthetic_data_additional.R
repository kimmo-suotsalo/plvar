# plot_results_synthetic_data_additional.R
# Copyright (C) 2021 Johan Pensar
# 
...
for (i in 1:length(m)){
  res <- readRDS(paste("results/sim_results_d", d,"_aps", aps[i], ".rds", sep = ""))
  tmp <- vector("list",6)
...
# Precision
pdf(paste("simulation_precision_rev.pdf", sep = ""), width = 8, height = 4.2)
...
for (j in 1:2){
  for (i in 1:3){
    ...
    if (j == 1){
      mtext(text = paste("q = ",aps[i], sep = ""),  side = 3, line = 1, cex = 0.8)
    }
    if (i == 1){
...
# Recall
pdf(paste("simulation_recall_rev.pdf", sep = ""), width = 8, height = 4.2)
par(mfrow = c(2,3), mai = c(0.3, 0.5, 0.3, 0.2))
...
# Running time
pdf(paste("simulation_time_rev.pdf", sep = ""), width = 8, height = 2.1)
...
for (i in 1:3){
  ...
  box(col="gray25", lwd = 1)
  if (i == 1){
    legend(x = "right", y = 10, legend = c("PLVAR", "SCAD", "LASSO"), col = brewer.pal(n = 3, name = 'Dark2'), lty = 1, lwd = 5, cex = 1, bty = "n") 
  }
  axis(1, at = c(2,7,12,17,22), labels = c("50","100","200","400","800"), col="gray25", col.ticks="gray25", col.axis="gray25")
  axis(2, at = s[1:(2+2)], labels = c("1","10","100","1000"), col="gray25", col.ticks="gray25", col.axis="gray25")
  mtext(text = paste("q = ",aps[i], sep = ""), side = 3, line = 1, cex = 0.8)
  if (i == 1){
...
pdf(paste("simulation_lag_length_rev.pdf", sep = ""), width = 8, height = 2.1)
par(mfrow = c(1,3), mai = c(0.3, 0.5, 0.3, 0.2))
